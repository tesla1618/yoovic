...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            'thumbnail' =>$request->has('image')? $this->upload(dir: 'product/thumbnail/', format: 'png', image: $request['image']):$request->existing_thumbnail,
            'digital_file_ready' => $digitalFile,
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $request->has('meta_image') ?$this->upload(dir: 'product/meta/', format: 'png', image: $request['meta_image']) : $request->existing_meta_image,
        ];
    }
